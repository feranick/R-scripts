...
m1=read.table(inputFile, header = FALSE, nrow =1)
x1 <- matrix(scan(inputFile, n = nrow(m1)*ncol(m1)), nrow(m1), ncol(m1), byrow = TRUE)
...
# Read data in matrix
m = read.table(inputFile, header = FALSE, skip =1)
x <- matrix(scan(inputFile, n = nrow(m)*ncol(m), skip=1), nrow(m), ncol(m), byrow = TRUE)

# Define new matrix
a=matrix(NA , (nrow(x))*(ncol(x)-1), 4)

# Define header for new matrix
colnames(a) <- c("",parValue,XName,YName)
print(colnames(a))
...
if(csvOut==TRUE){
    write.csv(a, file = outputFile, row.names = F)} else {
        write.table(a, file = outputFile, quote = FALSE, sep = "\t", col.names = T, row.names = F)}