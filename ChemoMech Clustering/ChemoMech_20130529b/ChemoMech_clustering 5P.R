...
sampleName<- "MHCQ-clan"

NumPar=5
 Par=c("Calcite","Quartz","H","M","h")
#Par=c("A",  "B", "C",  "D",  "E", "F")
...
dataset<-cbind(A,B,C,D,E)

elements<-cbind(A,B,C, D)
...
plot(A,D/C,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[3],ylab="M/H",xlim=c(min(A),max(A)),ylim=c(min(D),max(D)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)
