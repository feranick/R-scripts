...
Summary<-rbind(sortedMean,stdA,stdB,Vol.F)
...
plot(A,B,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[3],ylab=Par[4],xlim=c(min(A),max(A)),ylim=c(min(B),max(B)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)

plot(A,MH,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[1],ylab="M/H",xlim=c(min(A),max(A)),ylim=c(min(B),max(B)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)
