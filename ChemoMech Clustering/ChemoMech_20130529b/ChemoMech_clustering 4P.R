...
if(skimData==T){
	...
	plot(B,C, xlab=Par[1], ylab=Par[2], main="This data will be analyzed")
	}
...
dataset<-cbind(A,B,C,D)

elements<-cbind(A,B,C)
...
Summary<-rbind(sortedMean,stdA,stdB,stdC,Vol.F)
...
plot(B,C,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[2],ylab=Par[3],cex.lab=1.7,cex.main=2,cex.axis=1.7,cex=1.7)
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
#grid(NULL, lwd = 1,equilogs =FALSE)

for (i in 1:numPhase)
	{
	cov<-oSIGMA[2:3,2:3,i]
	mu<-sortedMean[2:3,i]
...
plot(B,C,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[2],ylab=Par[3],xlim=c(min(B),max(B)),ylim=c(min(C),max(C)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)

plot(A,C/B,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[1],ylab="M/H",xlim=c(min(A),max(A)),ylim=c(min(C),max(C)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)
