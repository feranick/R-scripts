...
oSIGMA<- array(0,c(NumParClust,NumParClust,numPhase))
...
Summary<-rbind(sortedMean,stdA,stdB,stdC,stdD,Vol.F)
...
plot(C,F,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[3],ylab=Par[6],cex.lab=1.7,cex.main=2,cex.axis=1.7,cex=1.7)
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
#grid(NULL, lwd = 1,equilogs =FALSE)
...
errbar(c(1:numPhase),sortedMean[3,],sortedMean[3,]+stdC,sortedMean[3,]-stdC,cex.lab=1.5,pch=16,xlab="Phase",ylab=Par[3],col=c(1:numPhase+1),cex=2,cex.axis=1.5,xaxt = "n");   axis(1, at = 1:numPhase,cex.axis=1.5);lines(sortedMean[3,])#;   title("C (A)",cex.main=1.8)

 errbar(c(1:numPhase),sortedMean[4,],sortedMean[4,]+stdD,sortedMean[4,]-stdD,cex.lab=1.5,pch=16,xlab="Phase",ylab=Par[6],col=c(1:numPhase+1),cex=2,cex.axis=1.5,xaxt = "n");   axis(1, at = 1:numPhase,cex.axis=1.5);lines(sortedMean[4,])#;   title("D (H)",cex.main=1.8)
...
plot(F,E,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[6],ylab=Par[5],xlim=c(min(F),max(F)),ylim=c(min(E),max(E)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)


plot(F,E/D,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[6],ylab="M/H",xlim=c(min(F),max(F)),ylim=c(min(E/D),max(E/D)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)

plot(C,E/D,col=Sort.Phase+1,type="p",pch=Sort.Phase+15,xlab=Par[3],ylab="M/H",xlim=c(min(C),max(C)),ylim=c(min(E),max(E)))
legend("bottomright", paste("Phase(", 1:numPhase,")"), col=1:numPhase+1,pch = 1:numPhase%%10+15, cex = 1.5)
grid(NULL, lwd = 1,equilogs =FALSE)
...
image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.7, xlab="um", ylab="um")
...
for (i in 1:numPhase)
	...
	image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=alpha(1:numPhase+1, 0.2), pch = 1:numPhase%%10+15, cex.lab=1.7,cex.main=1.5,main=paste("M/H <",MaxMH, "in phase",i,"(coverage:", signif(100*a/p, 3),"%)"))
...
image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.7,cex.main=1.5,main="M/H phase match", xlab="um", ylab="um")
#par(new=T)
...
for (i in 1:numPhase)
	...
	p<-length(which(MHp!=0))
	#image(interp(X,Y,Sort.Phase), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.7,main=paste("M/H >",MinMH, "in phase",i))
	image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=alpha(1:numPhase+1, 0.2), pch = 1:numPhase%%10+15, cex.lab=1.7,cex.main=1.5,main=paste("M/H >",maxh*100, "% of max MH", "in phase",i,"(coverage:", signif(100*a/p, 3),"%)"))
...
for (k in seq(lowh,highh,steph)){
MaxMH=max(MH)*k/100	
print(MaxMH)
image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.5,cex.main=1.5,main=paste("MH (",round(MaxMH,0),") > ",k,"% of Max MH (",round(max(MH),0),")"), xlab="um", ylab="um")
 #par(new=T)
 MH2<-ifelse((MH > MaxMH),MH,0)
 MH3<-ifelse( MH2!=0.00,1,0)
 image(interp(X,Y,MH3, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=c(alpha("red", 0), alpha("black", 0.7)), add=T)
...
for (i in 1:numPhase)
	{
	h2<-ifelse((F > Maxh) & (Sort.Phase == i),F,0)
	h3<-ifelse( h2!=0.00,1,0)
	hp<-ifelse((Sort.Phase == i),F,0)
	a<-length(which(h2!=0))
	p<-length(which(hp!=0))
	#image(interp(X,Y,Sort.Phase), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.7,main=paste("M/H >",MinMH, "in phase",i))
	image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=alpha(1:numPhase+1, 0.2), pch = 1:numPhase%%10+15, cex.lab=1.7,cex.main=1.5,main=paste("h >",maxh*100, "% of max h", "in phase",i,"(coverage:", signif(100*a/p, 3),"%)"))


	#par(new=T)
	image(interp(X,Y,h3, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=c(alpha("red", 0), gray(i/(numPhase*1.25))), add=T)
	#par(new=F)
	}


#image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=alpha(1:numPhase+1, 0.0), pch = 1:numPhase%%10+15, cex.lab=1.7,main=paste("h map"))
...
Maxh=max(F)*k/100
print(Maxh)	
image(interp(X,Y,Sort.Phase, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=1:numPhase+1, pch = 1:numPhase%%10+15, cex.lab=1.7,cex.main=1.5,main=paste("h (",round(Maxh,0),") > ",k,"% of Max h (",round(max(F),0),")"), xlab="um", ylab="um")

 #par(new=T)
 h2<-ifelse((F > Maxh),F,0)
 h3<-ifelse( h2!=0.00,1,0)
 image(interp(X,Y,h3, xo=seq(min(X), max(X), length=mapLength), yo=seq(min(X), max(Y), length=mapLength)), col=c(alpha("red", 0), alpha("black", 0.7)), add=T)
 #par(new=F)
}
